...
echo openspliceai predict -m "$MODEL_PATH" -o "$OUTPUT_PATH" -f $FLANKING_SIZE -i "$DATA_PATH" -t $THRESHOLD -D \
> "$OUTPUT_FILE" 2> "$ERROR_FILE"
openspliceai predict -m "$MODEL_PATH" -o "$OUTPUT_PATH" -f $FLANKING_SIZE -i "$DATA_PATH" -t $THRESHOLD -D \
> "$OUTPUT_FILE" 2> "$ERROR_FILE"