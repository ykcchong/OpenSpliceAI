...
def calculate_precision_recall(predicted_labels, true_labels, class_label):
    true_positives = np.sum((predicted_labels == class_label) & (true_labels == class_label))
    false_positives = np.sum((predicted_labels == class_label) & (true_labels != class_label))
    false_negatives = np.sum((predicted_labels != class_label) & (true_labels == class_label))
    ...
    for type in ['test']:
        ...
        with h5py.File(input_file, "r") as f:
            ...
            for idx in range(gene_num):
                label_decode = labels[idx].decode('ascii')
                seq_decode = seqs[idx].decode('ascii')
                # strand_decode = strands[idx].decode('ascii')
                label_decode = list(label_decode) 
                label_decode = [int(x) for x in label_decode]
                print("label_decode: ", len(label_decode))
                ...
                predicted_labels = np.argmax(y, axis=2)[0]
                print("predicted_labels: ", len(predicted_labels))
                print("predicted_labels: ", predicted_labels)

                precision_acceptor, recall_acceptor = calculate_precision_recall(predicted_labels, label_decode, 1)
                precision_donor, recall_donor = calculate_precision_recall(predicted_labels, label_decode, 2)
                precision_neither, recall_neither = calculate_precision_recall(predicted_labels, label_decode, 0)

                print("precision_acceptor: ", precision_acceptor)
                print("recall_acceptor: ", recall_acceptor) 
                print("precision_donor: ", precision_donor)
                print("recall_donor: ", recall_donor)
                print("precision_neither: ", precision_neither)
                print("recall_neither: ", recall_neither)


                # acceptor_prob = y[0, :, 1]
                # donor_prob = y[0, :, 2]
                # print("label: ", label_decode)
                # print("seq: ", seq_decode)
                # # print("strand: ", strand_decode)
