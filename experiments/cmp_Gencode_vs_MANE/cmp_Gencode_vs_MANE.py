...
def plot_comparison(data_dict, metrics, specie, target, split):
    ...
    for i, metric in enumerate(metrics):
        ...
        for (condition, (scratch_data, finetune_data)), color in zip(data_dict.items(), colors):
            ...
            ax.plot(epochs, scratch_data[metric], color=color, linestyle='--', label=f'MANE dataset {condition}')
            ax.plot(epochs, finetune_data[metric], color=color, linestyle='-', label=f'Gencode dataset {condition}')
    ...
    for specie in species:
        ...
        for split in ["TRAIN", "TEST"]:
            ...
            for condition in conditions:
                MANE_dataset = f'{base_path}/train_spliceai_default_no_paralog_removal_outdir/{specie}/flanking_{condition}/SpliceAI_{specie}_train_{condition}_0_rs22/0/LOG/{split}/'

                GENCODE_dataset = f'{base_path}/train_outdir/SpliceAI-keras_data/{specie}/flanking_{condition}/SpliceAI_{specie}_train_{condition}_0_rs22/0/LOG/{split}/'   
                
                MANE_dataset_data = process_directory(MANE_dataset)
                print("=========")
                GENCODE_dataset_data = process_directory(GENCODE_dataset)

                data_dict[condition] = (MANE_dataset_data, GENCODE_dataset_data)
                print(MANE_dataset_data.keys())
                print(GENCODE_dataset_data.keys())

                # Add data to combined DataFrame
                for metric in metrics:
                    print(f'{MANE_dataset_data[metric]}')
                    print(f'{GENCODE_dataset_data[metric]}')
