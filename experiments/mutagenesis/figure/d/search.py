...
def generate_dot_plot(score_change, output_file):
    ...
    # Label the dots where the y value is 3
    for i, count in enumerate(counts):
        if count == 3:
            plt.text(i, count, str(i), fontsize=8, ha='center', va='bottom')
            print(i)
...
def exp_3(fasta_file, models, model_type, flanking_size, output_dir, device):
    ...
    for raw_pos in tqdm(range(seq_length)):
        ...
        for mut_base in mutations:
            ...
            # calculate the mutation score differences
            acc_score_diff = [ref_acceptor_scores[i] - mut_acceptor_scores[i] for i in ref_acceptor_locs]
            don_score_diff = [ref_donor_scores[i] - mut_donor_scores[i] for i in ref_donor_locs]
            
            # get the count of score diffs > 0.5
            acc_change_df.loc[raw_pos, mut_base] = len([x for x in acc_score_diff if x > 0.5])
            don_change_df.loc[raw_pos, mut_base] = len([x for x in don_score_diff if x > 0.5])
