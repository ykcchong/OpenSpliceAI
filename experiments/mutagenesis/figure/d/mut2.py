...
def exp_3(fasta_file, models, model_type, flanking_size, output_dir, device, site):
    ...
    for raw_pos in tqdm(range(seq_length)):
        ...
        acc_scoring_position = np.argmax(ref_acceptor_scores)
        don_scoring_position = np.argmax(ref_donor_scores)
        ref_acceptor_score = ref_acceptor_scores[acc_scoring_position]
        ref_donor_score = ref_donor_scores[don_scoring_position]
        assert(np.argmax(ref_acceptor_scores) == acc_scoring_position)
        ...
        for mut_base in mutations:
            ...
            mut_acceptor_score = mut_acceptor_scores[acc_scoring_position]
            mut_donor_score = mut_donor_scores[don_scoring_position]
...
def mutagenesis():
    ...
    for model_type, flanking_size, site in itertools.product(model_types, flanking_sizes, sites):
        ...
        # Initialize params
        output_dir = f"/ccb/cybertron/smao10/openspliceai/experiments/mutagenesis/figure/d/results/exp_{exp_number}/{model_type}_{flanking_size}_{sample}_{site}"
        ...
        exp_3(fasta_file, models, model_type, flanking_size, output_dir, device, site)
