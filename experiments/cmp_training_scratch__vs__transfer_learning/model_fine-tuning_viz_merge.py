...
def plot_comparison(data_dict, metrics, specie):
    ...
    for i, metric in enumerate(metrics):
        ...
        for (condition, (scratch_data, finetune_data)), color in zip(data_dict.items(), colors):
            ...
            ax.plot(epochs, scratch_data[metric], color=color, linestyle='--', label=f'Scratch {condition}')
            ax.plot(epochs, finetune_data[metric], color=color, linestyle='-', label=f'Fine-tuned {condition}')
    ...
    plt.tight_layout()
    plt.savefig(f'{specie}_all_conditions_comparison.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def main():
    ...
    for specie in species:
        ...
        for condition in conditions:
            ...
            data_dict[condition] = (scratch_data, finetune_data)

            # Add data to combined DataFrame
            for metric in metrics:
                combined_df[f'{metric}_scratch_{condition}'] = scratch_data[metric]
                combined_df[f'{metric}_finetune_{condition}'] = finetune_data[metric]

        # Plot comparison for all conditions
        plot_comparison(data_dict, metrics, specie)

        # Save combined DataFrame
        combined_df.to_csv(f'{specie}_all_conditions_comparison.csv', index=False)
