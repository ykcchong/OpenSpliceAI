...
DATA_PATH="./data/toy/human/GCF_000001405.40_GRCh38.p14_genomic.fna"
ANNOTATION_PATH="./data/toy/human/MANE_subset${SUBSET_SIZE}.gff"
THRESHOLD=0.9
...
if [ "$MODEL_TYPE" == "pytorch" ]; then
    ...
    OUTPUT_PARENT_DIR="$NATIVE_DIR/results_${SUBSET_SIZE}/pytorch_MANE_sub${SUBSET_SIZE}_${FLANKING_SIZE}nt_anno${USE_ANNOTATION}"
    MODEL_ARG="-m $MODEL_PATH"
elif [ "$MODEL_TYPE" == "keras" ]; then
    SCRIPT_PATH="./experiments/benchmark_predict/spliceai_default_test.py"
    OUTPUT_PARENT_DIR="$NATIVE_DIR/results_${SUBSET_SIZE}/keras_MANE_sub${SUBSET_SIZE}_${FLANKING_SIZE}nt_anno${USE_ANNOTATION}"
    MODEL_ARG=""
else
