...
def plot_flanking(averages, metrics, metric_labels, subsets, outdir):
    ...
    for subset in subsets:
        ...
        for metric, label in zip(metrics, metric_labels):
            ...
            plt.title(f'[Predict] {label} vs. Flanking Size (Subset size: {subset})', fontsize=20)
...
def plot_subset(averages, metrics, metric_labels, outdir):
    ...
    for metric, label in zip(metrics, metric_labels):
        ...
        plt.title(f'[Predict] {label} vs. Input Size', fontsize=30, fontweight='bold')  # Larger title
