...
hf = h5py.File('/home/kchao10/data_ssalzbe1/khchao/data/train_test_dataset_MANE_test/dataset'
                + '_' + sys.argv[1]
                + '.h5', 'r')
...
for i in range(len(hf.keys())//2):
    x_key = f'X{i}'
    y_key = f'Y{i}'
    torch.from_numpy(hf[x_key][:]).shape
    torch.from_numpy(hf[x_key][:])[0].float()
    torch.from_numpy(hf[y_key][:]).shape

    x = torch.from_numpy(hf[x_key][:]).float()
    y = torch.from_numpy(hf[y_key][:])
    ...
    for d in d_index_positions:
        print("d: ", d)
        print("nucleotide_labels[d+1:d+3]: ", nucleotide_labels[d-2:d+4])

    # Get Acceptor motif:
    for a in a_index_positions:
        print("a: ", a)
        print("nucleotide_labels[a-2:a]: ", nucleotide_labels[a-2:a+4])
