...
def initialize_paths(output_dir, project_name, flanking_size, exp_num, sequence_length, loss_fun, random_seed):
    ...
    ####################################
    MODEL_VERSION = f"SpliceAI_{loss_fun}_{project_name}_{flanking_size}_{exp_num}_rs{random_seed}"
    ####################################
    # Modify the model verson here!!
    ####################################
...
def initialize_model_and_optim(device, flanking_size, pretrained_model_path=None):
...
def fine_tune(args):
    ...
    print("Pretrained model: ", pretrained_model, file=sys.stderr)
    ...
    model, criterion, optimizer, scheduler, params = initialize_model_and_optim(device, flanking_size, pretrained_model)
