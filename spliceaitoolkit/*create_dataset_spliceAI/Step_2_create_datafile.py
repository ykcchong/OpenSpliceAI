...
# Usage: python Step_2_create_datafile.py <train, test, all> <0, 1, all>
...
# define chromosome groups based on data mode
if sys.argv[1] == 'train':
...
# open sequence and splice table files
with open(splice_table, 'r') as fpr1, open(sequence, 'r') as fpr2:
    for line1 in fpr1:
        line2 = fpr2.readline()

        # parse data using regex to split lines
        data1 = re.split('\n|\t', line1)[:-1]
        data2 = re.split('\n|\t|:|-', line2)[:-1]

        # ensure data consistency between two sources
        assert data1[2] == data2[0]
        assert int(data1[4]) == int(data2[1])+CL_max//2+1
        assert int(data1[5]) == int(data2[2])-CL_max//2

        # filter data based on chromosome group and paralog status
        if (data1[2] not in CHROM_GROUP) or (sys.argv[2] != data1[1] and sys.argv[2] != 'all'):
            continue
...
with h5py.File(data_dir + 'datafile' + '_' + sys.argv[1] + '.h5', 'w') as h5f:

    # define a string data type for hdf5 datasets
    dt = h5py.string_dtype(encoding='utf-8')
    
    # create datasets in the ndf5 file for each type of data collected
